...
export const Footer: React.FC<Props> = (_props) => {
  ...
  const devEnv = process.env.VERCEL_ENV === 'development';
  const previewEnv = process.env.VERCEL_ENV === 'preview';

  const gitCommintSHA = devEnv
    ? 'main'
    : process.env.NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA;

  const swtichBranchUrl = previewEnv
    ? 'https://www.bibata.live'
    : 'https://dev.bibata.live';
      ...
      <div className='container mx-auto'>
        ...
        <div className='pb-4 lg:pb-12'>
          ...
          <div className='mt-3 flex items-center justify-start gap-2 font-mono tracking-tighter text-sm text-white/[.89]'>
            ...
            <Link
              target='_blank'
              href={`${swtichBranchUrl}${path}`}
              className='inline-flex gap-1 hover:text-white hover:underline'>
              {`Switch to ${previewEnv ? 'Upstream' : 'Development'}`}
            </Link>
          </div>
